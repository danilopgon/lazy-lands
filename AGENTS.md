...
For UI work, also inspect the shipped screens under `apps/web/app/[locale]/**` via the
`frontend-handoff-contract` skill's `route-map.md`.
...
| UI/UX visual direction            | `DESIGN.md`, then the shipped `apps/web/` screens |
...
### DESIGN.md is the design reference, not a separate prototype

`DESIGN.md` is the durable design system specification (tokens, typography, components,
motion). The former `handoff` prototype directory has been removed (Block 11); its
reference value now lives in `DESIGN.md`, the `frontend-handoff-contract` skill's
`route-map.md`, and the shipped `apps/web/` screens themselves. Build using React
components, TailwindCSS, and shadcn/ui.

### Frontend implementation requires the handoff contract

When implementing ANY frontend page, screen, or route, the `frontend-handoff-contract` skill
is MANDATORY. The workflow is: (1) read the shipped screen (or, for a new screen, `DESIGN.md`

- `PRODUCT.md` + the nearest shipped sibling), (2) extract a field-by-field checklist, (3)
  implement against the checklist, (4) perform adversarial self-review comparing implementation
  vs the checklist before declaring done. Implementations that skip this workflow or deviate
  without explicit justification are incomplete. See
  `.agents/skills/frontend-handoff-contract/SKILL.md` for the full contract.

### Orchestrator: frontend tasks require handoff context in SDD phases

When the orchestrator launches SDD phases (`sdd-spec`, `sdd-design`, `sdd-tasks`) for a
frontend task (any route in `apps/web/app/`), it MUST:

1. Read `.agents/skills/frontend-handoff-contract/references/route-map.md` to find the
   shipped screen for that route (or, for a new screen with no shipped precedent,
   `DESIGN.md` + `PRODUCT.md` + the nearest shipped sibling screen).
2. Read the shipped screen source (the `page.tsx` + its presentational components).
3. Read the shared production primitives under `apps/web/components/**` that the screen
   relies on (Field, Notice, LoadingScribe, OriginBadge, Modal, etc.).
...
This is non-negotiable. The handoff checklist is the visual specification. If the spec does
not include it, the spec is incomplete.
