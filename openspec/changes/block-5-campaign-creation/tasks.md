...
- [ ] 6.4 **[OPS — run at block close]** Hosted migration deployment is handled by `.github/workflows/deploy-migrations.yml` on merge to `main`, scoped only to `supabase/migrations/**`, gated by the `production` environment approval, using `supabase db push --dry-run` before `supabase db push`. Record the dry-run/apply outputs from that workflow in the PR. Do NOT add a second migration pipeline in this feature PR.
