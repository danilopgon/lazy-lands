...
| Unit 2 E2E (task 2.4.1) | `apps/web/tests/e2e/nav-pending.spec.ts` — 6 tests, one per route-declaration class (breadcrumb, card/list row, header nav, button-styled CTA) plus the idle-invariant and footprint guards. Executed against the maintainer's dev server on port 3000: 6 passed. Mutation-checked — forcing `LinkPending` never to reveal itself fails 5 of the 6, and the idle guard correctly stays green, so the suite discriminates rather than passing vacuously. Prefetch requests are aborted in the route handler instead of delayed: Next warms every in-viewport link, and a warm route resolves inside one frame, which is exactly the case the 150ms grace period must stay quiet for. The spec asserts the status node is visible, never that it animates, so it holds in every `data-motion` mode by construction — relevant because CI runs `playwright.config.ts`, which spawns its own server with `VISUAL_REGRESSION_TEST_MODE=true` and therefore renders `data-motion="off"`. That exact configuration was not executed locally. |
| Unit 2 browser acceptance | PARTIAL. The maintainer validated `data-motion="full"` at default viewport width against commit `b459d8d`. Task 2.4.2 stays open: it requires all three modes at <=900px, and `subtle`, `off`, OS reduced motion, and the narrow breakpoint were not covered. Task 2.4.1 is a Playwright artifact, not a manual check, and has not been written. |
...
| Unit 4 browser acceptance | PARTIAL. `full` at default width, validated by the maintainer at `b459d8d`. Task 4.4.1 stays open for `subtle`, `off`, OS reduced motion, and <=900px. |
...
| Unit 5 browser acceptance | PARTIAL. `full` at default width, validated by the maintainer at `b459d8d`. Task 5.4.1 stays open for `subtle`, `off`, OS reduced motion, and <=900px. The reduced-motion case matters most here: it is the only condition that exercises the stamp-centring fix. |
...
- [ ] 2.4.2 / 4.4.1 / 5.4.1 — `full` at default width is covered (maintainer, at `b459d8d`). Still
      open: `subtle`, `off`, OS reduced motion, and <=900px. How to reach each:
      - `off`: start the dev server with `VISUAL_REGRESSION_TEST_MODE=true`.
      - OS reduced motion: DevTools → Rendering → *Emulate CSS prefers-reduced-motion: reduce*. This
        drives both the CSS and the JS path, since `useMotionMode` reads `matchMedia`.
      - `subtle`: NOT reachable at runtime. `app/[locale]/layout.tsx:121-122` resolves the mode to
        `full` or `off` and there is no toggle by design. Flipping `data-motion` in DevTools moves
      ...
      - <=900px: DevTools responsive mode at the `llg` breakpoint.
