...
| Unit 5 browser acceptance | `full` at 900x900, measured over CDP on the demo review screen with three seeded proposals. Accept: the stamp renders **inside the card's bounding box** (not clipped by `overflow-hidden`), and the card is removed 1279ms after the stamp appears — `STAMP_LIFETIME_MS + CARD_EXIT_MS` is 1280ms, so timer authority holds to the frame. Dismiss: the blockquote reads as struck through at t=24ms while the card is still at **opacity 1.00**, confirming the 80ms lead the critique asked for, with removal at 255ms against a 220ms window. OS reduced motion validated separately by the maintainer, which is the condition that exercises the stamp-centring fix. Still open: `subtle` and `off`. |
