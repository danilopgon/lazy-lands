...
- Completed: Phases 1-4 plus PR #44 Codex and CodeRabbit review remediation
- Remaining: None in apply phase
- Remediation: Verification report findings addressed for strict TDD evidence, dynamic session display, active-memory source labels, explicit dismiss non-persistence assertion, scoped draft rewrite after partial processing, mount-safe draft loading, retryable create/retire failure feedback, best-effort draft persistence, accurate memory 404s, PATCH archive docs, and safe linked-session labels.
...
| PR #44 CodeRabbit remediation | `apps/web/app/[locale]/campaigns/[id]/memory/review/__tests__/page.test.tsx`, `apps/web/app/[locale]/campaigns/[id]/sessions/new/__tests__/page.test.tsx`, `apps/web/tests/memory/api.test.ts`, `apps/web/app/[locale]/campaigns/[id]/__tests__/page.test.tsx`, `services/api/tests/memory/test_memory_routes.py` | Integration + Unit | ✅ Baseline before new RED: targeted frontend suite 17 passed; backend memory route suite 6 passed | ✅ Written first: create/retire failure feedback, thrown draft write navigation, update 404 mapping, generic source label, and accurate backend 404 body assertions failed before implementation | ✅ Passed: targeted frontend suite 34 passed; backend memory route suite 6 passed | ✅ 5 paths: create failure, retire failure, draft write failure, update 404, campaign/memory 404 messages | ✅ Error handling remains retryable; no raw `source_session_id` shown; validation error message now lives on `MemoryFactValidationError` |
...
- PR #44 CodeRabbit RED: targeted frontend suite failed after adding tests for create/retire mutation errors, thrown draft writes, and update 404 mapping; backend memory route suite failed after asserting distinct campaign vs memory 404 messages.
- PR #44 CodeRabbit GREEN: targeted frontend suite passed with 34 tests; backend memory route suite passed with 6 tests after implementation.
- PR #44 CodeRabbit frontend gates: `pnpm --filter web lint`, `pnpm --filter web typecheck`, targeted suite, and full `pnpm --filter web test` passed: 51 files, 379 tests.
- PR #44 CodeRabbit backend gates: `uv run ruff check app/ tests/`, `uv run ruff format --check app/ tests/`, `uv run mypy app/ --ignore-missing-imports`, `uv run pytest tests/memory/test_memory_routes.py`, and full `uv run pytest -m "not dev_inference"` passed: 267 passed, 1 deselected.
...
- PR #44 CodeRabbit remediation: Memory Review now shows localized retryable errors for failed accept/edit and retire mutations without removing cards or active rows; Log Session treats draft storage as best-effort and always navigates after a saved session; backend create distinguishes missing campaigns from missing memory facts; the frontend memory API maps PATCH 404s to `MemoryFactNotFoundError`; campaign detail renders a generic linked-session label instead of raw IDs.
