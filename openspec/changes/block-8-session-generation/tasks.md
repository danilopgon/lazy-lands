...
- [x] 6.3 PR 1 backend gates completed: `pytest`, `ruff check app/ tests/`, `ruff format --check app/ tests/`, and `mypy app/ --ignore-missing-imports` from `services/api/`. Frontend gates (`pnpm typecheck`, `pnpm lint`, frontend tests, and frontend formatting checks) are deferred to PR 2.
